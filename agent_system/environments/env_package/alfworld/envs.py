...
class AlfworldWorker:
    ...
    def __init__(self, config, seed, base_env, assigned_game_file=None):
        """
        Args:
            assigned_game_file: If provided (for validation), this worker will only use this specific game file.
                               If None (for training), worker can access all games from base_env.
        """
        if assigned_game_file is not None:
            # Validation mode: create env with only this specific game
            self.env = base_env.init_env(batch_size=1, specific_game_file=assigned_game_file)
        else:
            # Training mode: create env with all available games
...
class AlfworldEnvs(gym.Env):
    def __init__(self, alf_config_path, seed, env_num, group_n, resources_per_worker, is_train=True, env_kwargs={}, start_idx=0):
        super().__init__()
        ...
        num_available_games = len(base_env.game_files)
        
        if is_train:
            # Training: multiple workers can sample from all games
            self.num_processes = env_num * group_n
            assigned_games = [None] * self.num_processes  # None means access all games
        else:
            # Validation: one worker per game, deterministic assignment
            assert group_n == 1, "group_n should be 1 if validation/test"
            self.num_processes = env_num * group_n
            assert start_idx + self.num_processes <= num_available_games, (
                f"start_idx ({start_idx}) + num_processes ({self.num_processes}) "
                f"exceeds available games ({num_available_games})."
            )
            assigned_games = [base_env.game_files[start_idx + i]
        ...
        for i in range(self.num_processes):
            worker = env_worker.remote(
                    ...
                    seed + (i // self.group_n), 
                    base_env, 
                    assigned_game_file=assigned_games[i]  # None for training, specific game for validation
                    )
...
def build_alfworld_envs(alf_config_path, seed, env_num, group_n, resources_per_worker, is_train=True, env_kwargs={}, start_idx=0):
    return AlfworldEnvs(alf_config_path, seed, env_num, group_n, resources_per_worker, is_train, env_kwargs, start_idx=start_idx)
