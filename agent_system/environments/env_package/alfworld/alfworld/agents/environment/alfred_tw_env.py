...
class AlfredTWEnv(object):
    ...
    def init_env(self, batch_size, specific_game_file=None):
        ...
        # Choose which game files to register
        if specific_game_file is not None:
            # Validation mode: register only this specific game
            games_to_register = [specific_game_file]
        else:
            # Training mode: register all available games
            games_to_register = self.game_files
        
        env_id = textworld.gym.register_games(
            games_to_register,  # Changed from self.game_files
            request_infos,
